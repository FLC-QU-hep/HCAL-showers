...
class DCGAN_D(nn.Module):
    ...
    def __init__(self, ndf):
        ...
        ### convolution
        self.conv1 = torch.nn.Conv3d(1, ndf, kernel_size=(10,5,5), stride=1, padding=0, bias=False)
        ## layer-normalization
        self.bn1 = torch.nn.LayerNorm([39, 21, 21]) 
        ## convolution
        self.conv2 = torch.nn.Conv3d(ndf, ndf*2, kernel_size=(6,3,3), stride=1, padding=0, bias=False)
        ## layer-normalization
        self.bn2 = torch.nn.LayerNorm([34, 19, 19])
        #convolution
        self.conv3 = torch.nn.Conv3d(ndf*2, ndf*4, kernel_size=(4,4,4), stride=1, padding=0, bias=False)
        ## layer-normalization
        self.bn3 = torch.nn.LayerNorm([31, 16, 16])
        #convolution
        self.conv4 = torch.nn.Conv3d(ndf*4, 1, kernel_size=(4,2,2), stride=(1,2,2), padding=0, bias=False)
        
        
        # Read-out layer : 1 * isize * isize input features, ndf output features 
        self.fc1 = torch.nn.Linear((28 * 8 * 8)+1, 200)
    ...
    def forward(self, x, energy):
        ...
        #Grand total --> size changes from (48, 25, 25) to (28, 8, 8)

        
        x = x.view(-1, 28 * 8 * 8)
        # Size changes from (28, 7, 7) to (1, 28 * 8 * 8) 
...
def specNorm(m):
    classname = m.__class__.__name__
    if classname.find('Conv') != -1 or classname.find('LayerNorm') != -1 or classname.find('Linear') != -1:
