...
class PostProcess_Size1Conv_EcondV2(nn.Module):
    def __init__(self, layer=48, isize=13, nc=2, ndf=128, bias=False, out_funct='relu'):
        ...
        self.bnco1 = torch.nn.LayerNorm([self.layer, self.isize, self.isize])
        
        self.conv2 = torch.nn.Conv3d(ndf+int(ndf/2), ndf, kernel_size=1, stride=1, padding=0, bias=bias)
        self.bnco2 = torch.nn.LayerNorm([self.layer, self.isize, self.isize])

        self.conv3 = torch.nn.Conv3d(ndf, ndf, kernel_size=1, stride=1, padding=0, bias=bias)
        self.bnco3 = torch.nn.LayerNorm([self.layer, self.isize, self.isize])

        self.conv4 = torch.nn.Conv3d(ndf, ndf, kernel_size=1, stride=1, padding=0, bias=bias)
        self.bnco4 = torch.nn.LayerNorm([self.layer, self.isize, self.isize])
    ...
    def forward(self, img, E_True=0):
        img = img.view(-1, 1, self.layer, self.isize, self.isize)
                
        econd = torch.cat((torch.sum(img.view(-1, self.layer*self.isize*self.isize), 1).view(-1, 1), E_True), 1)
        ...
        econd = econd.expand(-1, -1, self.layer, self.isize, self.isize)        
        ...
        return img.view(-1, 1, self.layer, self.isize, self.isize)